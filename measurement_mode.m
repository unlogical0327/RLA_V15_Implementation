...
%[ref_status,detected_ID,detected_reflector_polar,reflector_index]=identify_reflector_polar_mex(ref_gauss_data_fit,amp_thres,dist_thres,reflector_diameter,dist_delta,Lidar_data)
...
if ll>=15  && moving_estimate_matching ==1
%     Lidar_trace;
%     rotation_trace;
...
elseif ll>=15
    [xy_vel_acc]=vel_acc_calc(scan_freq,Lidar_trace,rotation_trace);
...
% Lidar_rotation = detected_pool_polar(:,1)-Lidar_angle';   % Lidar 
% Lidar_rotation = mean(Lidar_rotation)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
var=0.5;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Calculate lidar x and y w moving estimate
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%          if length(Lidar_trace)<10
%          [Lidar_x_wm,Lidar_y_wm,rotation] = calc_Lidar_location(Lidar_trace,reflect_pool_xy,reflect_pool_ID,detected_pool_polar,detected_pool_ID,dist_err,angle_err,comp_flag);
%          Lidar_update_xy_wm = [Lidar_x_wm Lidar_y_wm];
%          111
%          else
%              if Lidar_trace(end,1)>Lidar_trace(end-1,1)
%             [Lidar_x_wm,Lidar_y_wm,rotation] = calc_Lidar_location_w_displ(Lidar_trace,rotation_trace,reflect_pool_xy,reflect_pool_ID,detected_pool_polar,detected_pool_ID,dist_err,angle_err) 
%             Lidar_update_xy_wm = [Lidar_x_wm Lidar_y_wm];
%             222
%              else
%             [Lidar_x_wm,Lidar_y_wm,rotation] = calc_Lidar_location_w_displ(Lidar_trace,rotation_trace,reflect_pool_xy,reflect_pool_ID,detected_pool_polar,detected_pool_ID,dist_err,angle_err) 
...
%              end                
%          end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Calculate lidar x and y wo moving estimate
          [Lidar_x_wm,Lidar_y_wm,rotation] = calc_Lidar_location(Lidar_trace,reflect_pool_xy,reflect_pool_ID,detected_pool_polar,detected_pool_ID,dist_err,angle_err,comp_flag);
          Lidar_update_xy_wm = [Lidar_x_wm Lidar_y_wm];
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        xy_jump=400;   %250; to run mea on trail wo cali  % threshold to tell if there is an error jump, Not larger than speed
        delta_xy

    if (match_quality>1000 || delta_xy>xy_jump) && (Lidar_trace(end,1)~=0 && Lidar_trace(end,2)~=0)  %abs(reflector_rmse-reflector_rmse_old) > abs(reflector_rmse_old*rmse_thres_var) || delta_xy>xy_jump   % exceptional condition to error jump
...
%    smooth_step=1;
%    [Lidar_update_xy_wm] = data_smooth(Lidar_trace,Lidar_update_xy_wm,smooth_step)
...
%        Lidar_trace=[Lidar_trace;Lidar_update_xy];
        ...
        rotation_trace=[rotation_trace;theta_rot];    %theta_rot
...
if jump_flag==0
    ...
    elseif reflector_rmse>1 && reflector_rmse<10
        mea_status=1;
    elseif reflector_rmse>10 && reflector_rmse<999.99
        mea_status=2;
